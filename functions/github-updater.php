<?php
	...
	/*
	theme Name: GitHub Theme Updater
	Description: Updates themes that are hosted on GitHub.
	Version: 1.0
	Author: Codepress
	...
	*/
	...
	/**
	 * Register a new GitHub theme
	 *
	 * @param array $config
	 */
	function github_theme_updater_register( $config ) {
		new GitHub_Theme_Updater( $config );
	}
	...
	class GitHub_Theme_Updater {
		...
		public function __construct( $config ) {
			...
			add_filter( 'pre_set_site_transient_update_themes', array( $this, 'update_available' ) );
		...
		/**
		 * Retrieves the local version from the file header of the theme
		 *
		 * @since 1.0
		...
		/**
		 * Retrieves the remote version from the file header of the theme
		 *
		 * @since 1.0
		 * @return string|boolean
		 */
		...
		/**
		 * Hooks into pre_set_site_transient_update_themes to update from GitHub.
		 *
		 * @since 1.0
		...
		public function update_available( $transient ) {
			...
			if ( $local_version && $remote_version && version_compare( $remote_version, $local_version, '>' ) ) {
				...
				$transient->response[ $this->config->slug ] = (object) $theme;
			}
