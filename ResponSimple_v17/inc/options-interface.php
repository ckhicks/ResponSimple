	...
	foreach ( $options as $value ) {
		...
		case "images":
			...
			foreach ( $value['options'] as $key => $option ) {
				...
				$output .= '<div style="float:left;margin:0px 10px 10px 0px;width:80px;height:200px;display:block;background:url(' . esc_url( $option ) . ') center left repeat transparent !important;" class="of-radio-img-img' . $selected .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;"></div>';
			}
			break;
